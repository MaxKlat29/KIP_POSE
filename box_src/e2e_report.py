...
def patch_project_report(report_md, rows, overall, stamp):
    ...
    note = (f"_Numbers regenerated by `box_src/e2e_finish.sh` ({stamp}); "
            f"overall AR = {_f(overall.get('AR'), 3)}._")
    patched = src[:sep_idx + 1] + new_rows + src[j:]
    # drop ANY stale regeneration note + a trailing blank it left behind, so
    # repeated runs stay byte-stable (no blank-line accumulation). Idempotent.
    out_lines = []
    k = 0
    while k < len(patched):
        ln = patched[k]
        if ln.startswith("_Numbers regenerated by `box_src/e2e_finish.sh`"):
            k += 1
            # swallow exactly one immediately-following blank line if present
            if k < len(patched) and patched[k].strip() == "":
                k += 1
            continue
        out_lines.append(ln)
        k += 1
    # insert the fresh note (with one blank separator before it) right after the
    # new last table row.
...
def _ab_block(ab_result):
    """Render the A/B winning-config + decision-flags block for RESULTS_PHASE2,
    from the ab_result.json e2e_ab.py wrote. Returns '' if no A/B data."""
    if not ab_result or not os.path.isfile(ab_result):
        return ""
    try:
        d = json.load(open(ab_result))
    except Exception:
        return ""
    best = d.get("best_label", d.get("best_config", "?"))
    lines = ["\n## 1b. A/B over the levers (auto-best)\n",
             f"Auto-selected config: **{best}** "
             f"(mean AR = {d.get('best_mean_ar')}). Full comparison in "
             f"[[AB_LEVERS]].\n",
             "| config | mean AR |", "|---|---|"]
    cfgs = d.get("configs", {})
    for name in ("planar", "m2", "tta", "m2_tta"):
        if name in cfgs:
            mark = " ✅" if name == d.get("best_config") else ""
            lines.append(f"| {name} | {cfgs[name].get('mean_ar')}{mark} |")
    dec = d.get("decision", {})
    flags = dec.get("flags", [])
    lines.append("\n**Decision flags (next engineering step):**")
    if not flags:
        lines.append("- ✅ none — every trainable object clears its threshold; "
                     "the pipeline is finish-ready as-is.")
    for fl in flags:
        lines.append(f"- ⚠️ **{fl['flag']}** — {fl['reason']} → `{fl['action']}`")
    return "\n".join(lines) + "\n"


def write_results_phase2(out_md, rows, overall, mode, preds_note, stamp,
                         planar_refine, best_config=None, ab_result=None):
    base_tbl = []
    cmp_tbl = []
    for oid in TRAINABLE_OBJ_IDS:
        r = rows.get(oid) or {}
    ...
    cfg_note = f"_Best config (auto-selected): **{best_config}**_\n" if best_config else ""
    ab_block = _ab_block(ab_result)
...
def main():
    ...
    ap.add_argument("--best-config", default=None,
                    help="the A/B auto-best config name (planar/m2/tta/m2_tta)")
    ap.add_argument("--ab-result", default=None,
                    help="ab_result.json from e2e_ab.py (winning config + flags)")
    a = ap.parse_args()
    ...
    write_results_phase2(a.out_md, rows, overall, a.mode, a.preds_note, stamp,
                         a.planar_refine, best_config=a.best_config,
                         ab_result=a.ab_result)
